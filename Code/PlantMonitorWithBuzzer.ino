...
void setup() {
  ...
  // Set up LEDM
  // Initialize the LEDM pin as an output
  // Turn the LED off by making the voltage LOW
  pinMode(LEDM, OUTPUT);     
  digitalWrite(LEDM, LOW);
  
  // Set up LEDH
  // Initialize the LEDH pin as an output
  // Turn the LED off by making the voltage LOW
  pinMode(LEDH, OUTPUT);     
...
//Reconnct to MQTT if connection failed
  ...
  Serial.println(msg);
  client.publish("student/CASA0014/plant/ucfnnie/humidity", msg);

  //Moisture = analogRead(soilPin);   // moisture read by readMoisture function
  snprintf (msg, 50, "%.0i", Moisture);
  Serial.print("Publish message for m: ");
  Serial.println(msg);
  client.publish("student/CASA0014/plant/ucfnnie/moisture", msg);
...
// Switch on the Red LED for Moisture if a 2 was received as first character
  if ((char)payload[0] == '2') {
    BuzzerM(); //Turn on Buzzer to indicate the moisture is low
  } else {
    BuzzerC();  // Turn off the Buzzer
  }
  // Switch on the Yellow LED for humidity if a 3 was received as first character
  if ((char)payload[0] == '3') {
    BuzzerH();//Turn on Buzzer to indicate the humidity is low
  } else {
    BuzzerC(); //Turn off the Buzzer
  }
}
